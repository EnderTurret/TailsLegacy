...
public final class PartsPanel extends Panel {
	...
	public void renderButton(PoseStack poseStack, int mouseX, int mouseY, float partialTick) {
		...
		drawCenteredString(poseStack, parent.font(), TailsComponents.PART_SELECT, (right - left) / 2, 5, 0xFFFFFFFF);
	}
		...
		public void render(PoseStack poseStack, int slotIndex, int x, int y, int listWidth, int slotHeight, int mouseX, int mouseY, boolean isSelected, float partialTick) {
			...
			if (currentPart && editingInfo.getPartTexture() != null && editingInfo.getSubType() != null) {
				...
				if (textureAuthor != null || modelAuthor != null) {
					...
					poseStack.pushPose();
					poseStack.translate(5, x + 27 + nameOffset, 0);
					poseStack.scale(0.6F, 0.6F, 1);

					if (!twoAuthors) {
						drawString(poseStack, parent.font(), textureAuthor != null ? TailsComponents.TEXTURE_CREDIT : TailsComponents.PART_CREDIT, 0, 0, 0xFFFFFFFF);
						drawString(poseStack, parent.font(), new TextComponent(singleAuthor).withStyle(ChatFormatting.AQUA), 0, 10, 0xFFFFFFFF);
					} else {
						drawString(poseStack, parent.font(), TailsComponents.PART_CREDIT, 0, 0, 0xFFFFFFFF);
						drawString(poseStack, parent.font(), new TextComponent(modelAuthor).withStyle(ChatFormatting.AQUA), 0, 10, 0xFFFFFFFF);
						drawString(poseStack, parent.font(), TailsComponents.TEXTURE_CREDIT, 0, 20, 0xFFFFFFFF);
						drawString(poseStack, parent.font(), new TextComponent(textureAuthor).withStyle(ChatFormatting.AQUA), 0, 30, 0xFFFFFFFF);
					}
			...
			RenderHelper.drawScrollingString(poseStack, parent.font(), new TranslatableComponent(partInfo.getPart().getTranslationKey()), 5, listWidth - 8, x + 17 + nameOffset, 0xFFFFFFFF);
		}
