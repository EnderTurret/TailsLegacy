...
import net.enderturret.tailslegacy.common.client.TailsVec3f;
...
import net.enderturret.tailslegacy.common.client.model.ModelSerializer;
...
public class PartLoadingManager {
	...
	private Map<AttachmentPoint, List<TResourceLocation>> readOrdering(ResourceManagerWrapper manager) {
		...
		for (Map.Entry<String, List<String>> entry : rawOrdering.entrySet()) {
			...
			if (attachment == null)
				TailsPlatform.get().logError("part_ordering.json: Unknown attachment point: " + entry.getKey());
			else {
				final List<TResourceLocation> realValues = entry.getValue()
						...
						.map(str -> {
							try {
								...
								TailsPlatform.get().logError("part_ordering.json: Invalid resource location '{}'!\n{}", str, e.toString());
								return null;
							}
						})
						.filter(rl -> rl != null)
	...
	private List<Part> reloadParts(ResourceManagerWrapper manager) {
		...
		for (ResourcePair pair : orderings) {
			...
			if (!json.isJsonArray()) {
				TailsPlatform.get().logError("Texture ordering {} must be a json array!", pair.location);
				continue;
			}
		...
		for (ResourcePair pair : textures) {
			...
			if (!json.isJsonObject()) {
				TailsPlatform.get().logError("Texture {} must be a json object!", pair.location);
				continue;
			}
			...
			if (tex.applyTo.isEmpty())
				TailsPlatform.get().logError("Texture {} does not apply to any sub types!", pair.location);
		}
		...
		for (ResourcePair pair : subTypes) {
			...
			if (json == null) continue;
			if (!json.isJsonObject()) {
				TailsPlatform.get().logError("Sub type {} must be a json object!", pair.location);
				continue;
			}
		...
		for (ResourcePair pair : parts) {
			...
			if (json == null) continue;
			if (!json.isJsonObject()) {
				TailsPlatform.get().logError("Part {} must be a json object!", pair.location);
				continue;
			}
			...
			try {
				...
				TailsPlatform.get().logError("Failed to read part {}:", pair.location, e);
				continue;
			}
	...
	private static NamedSubType readSubType(TResourceLocation location, JsonObject json, List<NamedTexture> textures, Map<TResourceLocation, List<String>> textureOrderings) {
		...
		final List<PartPath> hideParts = json.has("hideParts") ? getAsStringArray(json, "hideParts").stream().map(PartPath::new).collect(Collectors.toList()) : Collections.emptyList();
		final List<PartPath> showParts = json.has("showParts") ? getAsStringArray(json, "showParts").stream().map(PartPath::new).collect(Collectors.toList()) : Collections.emptyList();
