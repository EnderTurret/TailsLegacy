...
public class TexturePanel extends Panel<GuiEditor> {
    ...
    public void init() {
        ...
        addButton(rightBtn = new ExtendedButton(width - 20, texSelectX, 15, 15, new StringTextComponent(">"), b -> {
        	PartInfo originalPartInfo = parent.getEditingPartInfo();
            RenderPart part = PartRegistry.getRenderPart(parent.getPartType(), originalPartInfo.typeid);
    ...
    public void render(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks) {
        ...
        setBlitOffset(-5);
        fillGradient(matrixStack, 7, texSelectX, width - 15, texSelectX + 15, 0x55000000, 0x55000000); //Use gradientRect so it actually takes into account zlevel

        //Texture select
        drawCenteredString(matrixStack, font, I18n.format("gui.texture"), width/2, texSelectX - 12, 0xFFFFFF);
